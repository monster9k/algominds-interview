...
**Ngoại lệ đã có — `client/src/features/admin/pages/admin-problems-page.tsx` + `admin-problems-table.tsx`:** theo yêu cầu tường minh của user, 2 file này dùng skill `taste-skill` (`.claude/skills/taste-skill/`, vendor từ `Leonxlnx/taste-skill`, MIT) thay vì `frontend-design` — font Geist Sans/Geist Mono (`@fontsource/geist-sans`, `@fontsource/geist-mono`) và icon Phosphor (`@phosphor-icons/react`) + `motion` chỉ scope trong 2 file này qua `style={{fontFamily: ...}}` inline, KHÔNG đổi `index.css`/font toàn app, KHÔNG đổi icon set ở các trang admin khác (`ConfirmDialog`/`AdminPagination` dùng chung vẫn `lucide-react`). Lưu ý quan trọng khi đọc code 2 file này: skill `taste-skill` tự ghi rõ trong chính nó là **không dành cho dashboard/data table** (section Preflight + checklist cuối file) — việc áp dụng ở đây là override có chủ đích của user sau khi đã được cảnh báo, không phải use-case skill này tự nhận là phù hợp. **Không tự ý lan rộng font Geist/icon Phosphor sang các trang admin khác** khi làm task mới — mỗi lần muốn mở rộng phải hỏi lại user, vì đây là ngoại lệ cục bộ có ghi chú rõ, không phải hướng đi mới của cả app.
