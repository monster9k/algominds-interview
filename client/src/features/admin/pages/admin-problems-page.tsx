...
import {
  AdminProblemsTable,
  ProblemSortBy,
} from "../components/admin-problems-table";
...
export function AdminProblemsPage() {
  ...
  const toggleSelectAll = () => {
    ...
    const allSelected =
      selectable.length > 0 && selectable.every((p) => selectedIds.has(p.id));
    setSelectedIds(
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await Promise.all(
        [...selectedIds].map((id) => deleteProblem.mutateAsync(id)),
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmDialog
        ...
        title={t("problems.bulkDeleteConfirmTitle", {
