...
- [x] **FE: `/admin/problems` — Create/Edit dialog + Delete + pagination/search**
  📍 `admin-problems-page.tsx`/`admin-problems-table.tsx` — `useAdminProblems({page,search})` (gọi `GET /admin/problems`), ô search + `AdminPagination`, cột Trạng thái đọc `deletedAt` thật (Active/Deleted, khắc phục hạn chế đã ghi nhận ở P0). `problem-form-dialog.tsx` dùng chung Create/Edit, JSON textarea có validate parse trước khi submit; Edit fetch chi tiết đầy đủ qua `GET /admin/problems/:id` (thêm mới) để prefill kể cả `hiddenTestCases`. Verify end-to-end qua browser thật: tạo → search tìm thấy → sửa title → xoá → badge Trạng thái đổi "Deleted" đúng ngay tại chỗ (row vẫn hiển thị, nút Delete tự disable).

- [x] **FE: `/admin/contests` — Create/Edit dialog + Delete + pagination/search**
  📍 `useAdminContests(...)`, `contest-form-dialog.tsx` (title/description/startTime/endTime + `problemCounts.easy/medium/hard` chỉ hiện ở Create, đúng cơ chế random-pick hiện có, KHÔNG chọn tay problem). Edit không cần endpoint chi tiết riêng — list item đã đủ field (thêm `description` vào select ở task BE trước) để prefill.
  **Bug thật bắt được lúc test browser**: `toDatetimeLocal()` ban đầu chỉ `.slice(0,16)` chuỗi ISO (UTC) thay vì đổi sang giờ local — datetime hiển thị lại ở form Edit lệch múi giờ so với lúc nhập (vd nhập 10:00 sáng giờ VN, lưu 03:00 UTC, Edit hiện lại "03:00" thay vì "10:00"). Đã sửa dùng `Date` getter local (`getFullYear/getMonth/getDate/getHours/getMinutes`) thay vì slice thô. Verify end-to-end qua browser thật: tạo (kèm bug trên, phát hiện lúc mở lại form Edit) → sửa lại code → edit title → xoá → badge Trạng thái đổi "Deleted" đúng, dữ liệu thời gian khớp lại 100% sau khi sửa.
