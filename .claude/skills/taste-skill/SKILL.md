...
source: Vendored verbatim from Leonxlnx/taste-skill (skills/taste-skill/SKILL.md), MIT licensed (see LICENSE in this folder). NOTE — this skill's own scope note (Section "Preflight", line ~8 and ~899-900) explicitly excludes dashboards, admin panels, and data tables. AlgoMinds' /admin/* pages are exactly that; using this skill there was an explicit user override of that scope note, not a fit the skill claims for itself. See .claude/rules/design.md for how that override was scoped (admin/problems page only, not app-wide).
---
